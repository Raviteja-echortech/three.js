...
scene.add(box);
loader.load('/ironman', function (object) {
    object.postion.z -= 60
    scene.add(object)
})
const PlaneGeometry = new THREE.PlaneGeometry(30, 30);
const planeMaterial = new THREE.MeshStandardMaterial({ color: "white", side: THREE.DoubleSide, map: spaceTexture.load(space) })
...
//random arrangement of sphere or(starts) in the space

function addStart() {
    const spearGeomentry = new THREE.SphereGeometry()
    const spearMesh = new THREE.MeshStandardMaterial({ map: spaceTexture.load(space), })
    const sphere = new THREE.Mesh(spearGeomentry, spearMesh)
    scene.add(sphere)
